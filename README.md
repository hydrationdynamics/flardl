...
While the _mean_ per-file download rate isn't a good statistic, the
_modal_ per-file download rate $k^\prime_{\rm file}$ can be
...
slowing is usually because bit rate of your LAN has to be shared
between two demand streams. Higher predictability and performance
...
$$
  k^{\prime}_{file} = 1 / (t_{\rm lat} + (1 + c_{\rm prot}) S^{\prime}/B_{\rm act})
$$
